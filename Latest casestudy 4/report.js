  ...
  fetch("retrieveprice.php")
    ...
    .then((coffeeprice) => {
      coffeeprice.forEach((coffee) => {
        if (coffee.ID == 1) {
          newjjprice = coffee.price;
        } else if (coffee.ID == 2) {
          newcal1price = coffee.price;
        } else if (coffee.ID == 3) {
          newcal2price = coffee.price;
        } else if (coffee.ID == 4) {
          newcap1price = coffee.price;
        } else if (coffee.ID == 5) {
          newcap2price = coffee.price;
        }
      });
    })
    .catch(console.error);
...
function productreport() {
  ...
  fetch("getorders.php")
    ...
    .then((orders) => {
      ...
      jjdollar = parseFloat(jj) * newjjprice;
      caldollar =
        parseFloat(cal1 * newcal1price) + parseFloat(cal2 * newcal2price);
      capdollar =
        parseFloat(cap1 * newcap1price) + parseFloat(cap2 * newcap2price);
...
function categoryreport() {
  ...
  fetch("getorders.php")
    ...
    .then((orders) => {
      ...
      jjdollar = parseFloat(jj) * newjjprice;
      singledollar =
        parseFloat(cal1 * newcal1price) + parseFloat(cap1 * newcap1price);
      doubledollar =
        parseFloat(cal2 * newcal2price) + parseFloat(cap2 * newcap2price);
